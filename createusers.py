...
with open(filename, 'r') as f:
  ...
  for row in reader:
    ...
    newFirst = row[3]
    newLast = row[4]
    print(f"Username: {newUserUsername}, Email:{newUserEmail}, Group:{newGroup}, First:{newFirst}, Last:{newLast}")
...
with open(filename, 'r') as f:
  ...
  for row in reader:
    ...
    print(email)
    for node1 in data1[1]:
      #print(node1['node']['email'])
      if node1['node']['email'] == email:
        # Print the node
        #print(node1['node']['id'])
        emailnodeid = (node1['node']['id'])
        #print(emailnodeid)
